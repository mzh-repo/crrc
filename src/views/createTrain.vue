      ...
      >
        <el-collapse-item :name="index">
          ...
          <el-row @click.native="setSelect(item.id)">
            <el-col v-for="(histogramList, i) in item.graph" :span="8" :key="i" class="echarts">
              <histogram :colors="colors[i]" :title="titleList[i]" :lineData="histogramList" />
            </el-col>
            <img v-if="item.id === selected" src="@/assets/images/choiced.png" class="stamp" />
...
export default {
  ...
  methods: {
    ...
    getlineDataList() {
      this.$axios.get(`/dataset/graph?database_id=${this.databaseId}`).then((res) => {
